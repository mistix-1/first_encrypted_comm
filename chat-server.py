...
# Get the directory where the script is running, to locate other files.
...
@app.route('/download/alice', methods=['GET'])
def download_alice_client():
    """Serves the alice-client.py file for download."""
    try:
        print("Request to download alice-client.py received.")
        return send_from_directory(APP_ROOT, 'alice-client.py', as_attachment=True)
    except FileNotFoundError:
        return jsonify({"status": "alice-client.py not found on server."}, 404)

@app.route('/download/bob', methods=['GET'])
def download_bob_client():
    """Serves the bob-client.py file for download."""
    try:
        print("Request to download bob-client.py received.")
        return send_from_directory(APP_ROOT, 'bob-client.py', as_attachment=True)
    except FileNotFoundError:
        return jsonify({"status": "bob-client.py not found on server."}, 404)

@app.route('/download/encryption', methods=['GET'])
def download_encryption_lib():
    """Serves the encryption_code.py file for download."""
    try:
        print("Request to download encryption_code.py received.")
        return send_from_directory(APP_ROOT, 'encryption_code.py', as_attachment=True)
    except FileNotFoundError:
        return jsonify({"status": "encryption_code.py not found on server."}, 404)
